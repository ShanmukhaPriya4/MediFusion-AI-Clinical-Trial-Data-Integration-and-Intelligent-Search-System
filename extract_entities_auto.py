...
for input_file in batch_files:
    ...
    if not match:
        print("Skipping file (pattern not matched)")
        continue

    batch_number = match.group(1)

    json_output = os.path.join(
        output_folder, f"drug_therapeutic_area{batch_number}.json"
    )
    csv_output = os.path.join(
        output_folder, f"drug_therapeutic_area{batch_number}.csv"
    )
    ...
    print(f"Total records in batch {batch_number}: {total_records}")
